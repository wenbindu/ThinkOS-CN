...
%time is
同样的, 我们可以将``未命中惩罚''定义为处理未命中缓存的额外时间, 
...
的``块'' 或``行'' 数据. 如果程序继续读取一个相邻字节, 那么已经在缓存中了.
...
的首个字节恰好位于块的开头. 当你加载第一个字节时, 会遇到缺失惩罚, 
但之后的整个字符串都会存在于缓存中. 读取整个字符串, 命中率将会是
63/43, 大约 98\%. 如果字符串横跨两个块, 你会遇到2次缺失惩罚. 
