...
检查队列是否已满之前，我们需要先锁定 {\tt Mutex}。  
如果队列已满，则在退出之前必须先解锁 {\tt Mutex}；否则，
该线程将保持锁定状态，其他线程将无法继续执行。

\verb"queue_pop" 的同步代码与此类似：
...
注意，其他 {\tt Queue} 相关函数——包括 \verb"queue_full"、\verb"queue_empty" 和 \verb"queue_incr"——都不会尝试去锁定互斥锁。  
任何调用这些函数的线程都需要先手动加锁；这是这些函数接口文档中的一部分约定。

通过添加这些代码，现在队列已经是线程安全的；如果你运行它，应该不会再看到任何同步错误。  
但是，很可能在某个时刻消费者会因为队列为空而退出，或者生产者因为队列已满而退出，或者两者都会发生。
...
条件变量是一种与“条件”相关联的数据结构；它允许线程在条件变为真之前一直阻塞等待。  
例如，\verb"thread_pop" 可能想先检查队列是否为空，如果为空，就等待“队列非空”这一条件。

类似地，\verb"thread_push" 可能需要检查队列是否已满，如果已满，就阻塞直到队列不再是满的。

这里我先处理第一个条件，而第二个条件将作为练习留给你完成。

首先，我们在 {\tt Queue} 结构中添加一个条件变量：
...
现在，在 \verb"queue_pop" 中，如果发现队列为空，我们不再直接退出；而是使用条件变量进行阻塞等待：
...
\verb"cond_wait" 的行为比较复杂，因此我们一步一步来看。  
第一个参数是条件变量，在这里我们等待的条件是“队列非空”。  
第二个参数是用于保护队列的互斥锁。

当已经锁定互斥锁的线程调用 \verb"cond_wait" 时，它会先解锁互斥锁，然后进入阻塞状态。  
这一点非常重要。  
如果 \verb"cond_wait" 在阻塞前没有释放互斥锁，那么其他线程将无法访问队列，也就无法继续添加新元素，队列将永远保持为空。

因此，当消费者线程阻塞在 {\tt nonempty} 上时，生产者线程是可以继续运行的。  
来看一下生产者运行 \verb"queue_push" 时发生了什么：
...
与之前一样，\verb"queue_push" 会先锁定 {\tt Mutex}，并检查队列是否已满。  
在假设队列未满的情况下，它会向队列中添加一个新元素，然后解锁 {\tt Mutex}。

但在返回之前，它还会多做一件事：对条件变量 {\tt nonempty} 进行一次“信号通知（signal）”。
...
对条件变量发出信号，通常意味着对应的条件已经为真。  
如果此时没有线程在该条件变量上等待，那么该信号不会产生任何影响。

如果有线程正在该条件变量上等待，则其中一个线程会被唤醒，并继续执行 \verb"cond_wait" 之后的代码。  
但是，在被唤醒的线程真正从 \verb"cond_wait" 返回之前，它必须重新获取并锁定 {\tt Mutex}。
...
现在回到 \verb"queue_pop"，看看线程从 \verb"cond_wait" 返回时会发生什么。  
它会回到 while 循环的开头，再次检查条件。  
稍后我会解释原因，但现在先假设条件为真，也就是说队列中已经有元素。

当消费者线程跳出 while 循环时，我们可以确定两点：  
(1) 条件为真，即队列中至少有一个元素；  
(2) {\tt Mutex} 已经被锁定，因此可以安全地访问队列。

在取出一个元素之后，\verb"queue_pop" 会解锁互斥锁并返回。

在下一节中，我会展示 {\tt Cond} 的具体实现方式，但在此之前，我想先回答两个常见问题：
...
\item 为什么 \verb"cond_wait" 要放在 while 循环中，而不是 if 语句中？
也就是说，为什么从 \verb"cond_wait" 返回后还需要再次检查条件？
...
%cause an error.
主要原因在于“信号被截获”的可能性。  
假设线程 A 正在 {\tt nonempty} 上等待。  
线程 B 向队列中添加了一个元素，并对 {\tt nonempty} 发出了信号。  
线程 A 被唤醒并试图获取互斥锁，但在它成功获取之前，邪恶的线程 C 抢先一步，锁定互斥锁，从队列中取走了这个元素，然后解锁。  
此时队列再次变为空，但线程 A 已经不再处于阻塞状态。  
线程 A 随后获取互斥锁，并从 \verb"cond_wait" 返回。  
如果线程 A 不再次检查条件，它就会从一个空队列中取数据，从而很可能导致错误。
...
%is associated with?''
\item 另一个常见问题是：“条件变量如何知道它对应的具体条件是什么？”

这是可以理解的，因为在 {\tt Cond} 结构体与其所关联的条件之间，并不存在显式的绑定关系。  
它们之间的关联是通过使用方式隐式建立的。

可以这样理解：  
当你调用 \verb"cond_wait" 时，条件是“假”的；  
当你调用 \verb"cond_signal" 时，条件是“真”的。
...
由于线程在从 \verb"cond_wait" 返回后都会重新检查条件，因此实际上并不要求只在条件一定为真时才调用 \verb"cond_signal"。  
只要你有理由认为条件“可能”为真，就可以调用 \verb"cond_signal" 作为一种提示，提示现在是检查条件的合适时机。
...
%
%Here's the typedef:
前一节中使用的 {\tt Cond} 结构，是对 POSIX 线程 API 中 \verb"pthread_cond_t" 类型的一个封装。  
这与 {\tt Mutex} 非常相似，{\tt Mutex} 是 \verb"pthread_mutex_t" 的封装。  
这两个封装都定义在 {\tt utils.c} 和 {\tt utils.h} 中。
...
\verb"make_cond" 负责分配空间、初始化条件变量，并返回一个指针：
...
下面是 \verb"cond_wait" 和 \verb"cond_signal" 的封装实现：
